...
let currentData = {
    ...
    mergedCells: [],
    cellFormats: {},
    subjectNameSettings: {}, // Для направлений
    subgroupSettings: {}     // Для подгрупп (хранение настроек для добавления названия дисциплины и направления)
};
...
// Функция для добавления всех преподавателей из встроенного списка
function addAllTeachersFromList() {
    ...
    BUILT_IN_TEACHERS.forEach((teacherName, index) => {
        ...
        if (!currentData.teachers.some(t => t.name === teacherName)) {
            const teacher = {
                id: baseTime + index, // Добавляем смещение, чтобы избежать одинаковых ID
                name: teacherName,
                selected: true
            };
    ...
    if (addedCount > 0) {
        showMessage(`Добавлено преподавателей: ${addedCount}${skippedCount > 0 ? `, пропущено (уже есть): ${skippedCount}` : ''}`, 'success');
    } else {
        showMessage('Все преподаватели из списка уже добавлены', 'info');
    }
    
    // Очищаем поле ввода и выбор
...
function showMessage(message, type = 'success') {
    ...
    setTimeout(() => {
        ...
        errorDiv.style.background = '#e74c3c'; // Возвращаем исходный цвет для ошибок
    }, 3000);
}
...
function handleFileUpload() {
    ...
    currentData.subgroupSettings = {}; // Сбрасываем настройки подгрупп
        ...
        // Проверяем, является ли строка новым предметом
            ...
            if (i + 1 < rows.length) {
                ...
                if (hasNoDirectionCode && hasSeminars) {
                    ...
                    // Инициализируем настройку для подгруппы (по умолчанию false)
...
// Функция для переключения настройки добавления названия дисциплины для направления
...
// Функция для переключения настройки добавления названия дисциплины и направления для подгруппы
...
// Функция для обновления отображения чекбокса направления
...
// Функция для обновления отображения чекбокса подгруппы
...
// Функция для добавления формул в столбец "Итого часов" (AI)
function addTotalFormulas(ws, totalRows, headerRowsCount = 4) {
    // Проходим по всем строкам данных (после заголовков)
    for (let R = headerRowsCount; R < totalRows; R++) {
        const rowNum = R + 1; // Номер строки в Excel (1-based)
        // Формула суммирования нужных столбцов: I, J, K, Q, R, S, AG, AH
        const formula = `=I${rowNum}+J${rowNum}+K${rowNum}+Q${rowNum}+R${rowNum}+S${rowNum}+AG${rowNum}+AH${rowNum}`;
        const cellAddress = XLSX.utils.encode_cell({ r: R, c: 34 }); // столбец AI (индекс 34)
        
        // Если ячейка ещё не существует, создаём её
        if (!ws[cellAddress]) {
            ws[cellAddress] = {};
        }
        // Устанавливаем тип "число" и формулу
        ws[cellAddress].t = 'n';
        ws[cellAddress].f = formula;
        // Очищаем значение, чтобы Excel вычислил формулу при открытии
...
function createTeacherSheet(teacher, assignments) {
    ...
    for (let i = 0; i < currentData.headerRows.length; i++) {
        ...
        // Пустые ячейки просто пропускаем (не добавляем в массив)
    }
    ...
    // Добавляем формулы для столбца "Итого часов" (начиная с 5-й строки)
...
function exportTeacherAssignments(ws, exportData, startRow, assignments) {
    ...
    for (let i = 0; i < rowsToExport.length; i++) {
        ...
        // Создаем новую строку ТОЛЬКО с теми колонками, где есть данные
        const newRow = [];
        
        // Копируем только непустые значения из оригинальной строки (до 35 колонок)
        for (let col = 0; col < 35 && col < originalRow.length; col++) {
            const value = originalRow[col];
            // Добавляем только если значение существует и не является пустым
            if (value !== undefined && value !== null && value !== "" && value !== " " && value !== "\u00A0") {
                newRow[col] = value;
            }
            // Если значение пустое - не добавляем ничего (ячейка будет отсутствовать)
        }
        
        // Если нужно добавить название дисциплины (для направления)
        ...
        // Если нужно добавить название дисциплины и направление (для подгруппы)
        if (rowData.isSubgroup) {
            const shouldAddForSubgroup = currentData.subgroupSettings[rowData.subgroupId] || false;
            
            if (shouldAddForSubgroup) {
                // Добавляем название дисциплины в колонку D (индекс 3)
                if (rowData.subjectName && (!newRow[3] || newRow[3] === "")) {
                    newRow[3] = rowData.subjectName;
                }
                
                // Добавляем код направления в колонку E (индекс 4)
        ...
        // Преобразуем "Итого часов" в число, если нужно
        ...
        // Добавляем строку в данные для экспорта ТОЛЬКО если в ней есть хоть какие-то данные
        const hasData = newRow.some(cell => cell !== undefined && cell !== null && cell !== "");
        if (hasData) {
            exportData.push(newRow);
            rowsAdded++;

            // Копируем стили ТОЛЬКО для существующих ячеек
...
// Новая функция для копирования стилей только для непустых ячеек
function copyRowStylesNonEmpty(ws, sourceRowIndex, targetRowIndex, originalRow) {
    // Проходим только по тем колонкам, где есть данные в оригинале
    for (let col = 0; col < originalRow.length; col++) {
        const value = originalRow[col];
        
        // Копируем стиль только если в ячейке было значение
        if (value !== undefined && value !== null && value !== "" && value !== " " && value !== "\u00A0") {
            ...
            // Создаем ячейку только если есть стиль
...
function copyRowStyles(ws, sourceRowIndex, targetRowIndex, colCount) {
    ...
    for (let col = 0; col < colCount; col++) {
        ...
        // Копируем стиль только если есть что копировать
        if (currentData.cellStyles[sourceCellAddress]) {
            // Создаем ячейку только если есть данные или стиль
            if (!ws[targetCellAddress]) {
        ...
        // Если ячейка была создана, но у нее нет значения - устанавливаем null
